  ...
  if (type === "haukiem" || type === "atgt") {
    return {
      summary: (results?.summary || []).map(row => ({
        ...
        atgt: Number(row.atgt) || 0,
        cldv: Number(row.cldv) || 0,
      })),
      detailRows: (results?.detailRows || []).map(({ sourceRow, ...row }) => row),
      violations: (results?.violations || []).map(({ sourceRow, matchedM02Row, ...row }) => row),
      fileSummaries: results?.fileSummaries || [],
      unmatchedCount: Number(results?.unmatchedCount) || 0,
      ignoredOtherCount: Number(results?.ignoredOtherCount) || 0,
    };
  }
  return results || [];
}
