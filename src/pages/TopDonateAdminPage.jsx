...
import { deleteTopDonate, getTopDonates, saveTopDonate, setTopDonateVisibility, sortTopDonates } from "../services/topDonateService";
...
export default function TopDonateAdminPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(item => `${item.donorName} ${item.message}`.toLocaleLowerCase("vi").includes(search));
  }, [items, keyword]);
  ...
  return <section className="mx-auto max-w-6xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.17em] text-amber-600 dark:text-amber-300"><Trophy size={16}/>Admin &amp; SuperAdmin</span><h1 className="mt-3 text-3xl font-bold text-ink dark:text-white">Quản lý Top Donate</h1><p className="mt-2 max-w-2xl text-sm text-slate-500 dark:text-slate-400">Thêm, chỉnh sửa, ẩn/hiện và xóa người Donate. Bảng xếp hạng tự sắp theo số tiền từ cao xuống thấp.</p></div><button className="primary-button" onClick={() => setEditor(null)}><Plus size={18}/>Thêm người Donate</button></div>
