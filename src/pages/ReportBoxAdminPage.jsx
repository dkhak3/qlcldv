...
import { Bus, Camera, ChartNoAxesCombined, ChevronDown, Clapperboard, ClipboardCheck, Edit3, Eye, EyeOff, FileSpreadsheet, Gauge, Headphones, LoaderCircle, LockKeyhole, MapPin, Plus, Radio, Save, Search, ShieldCheck, Trash2, Video, Wrench, X } from "lucide-react";
import { toast } from "react-toastify";
import ConfirmDialog from "../components/ConfirmDialog";
import { CORE_REPORT_BOX_COUNT, REPORT_APPEARANCES, isCoreReportBox } from "../data/reportBoxes";
...
const ICONS = { camera: Camera, "map-pin": MapPin, "file-spreadsheet": FileSpreadsheet, gauge: Gauge, headphones: Headphones, video: Video, bus: Bus, chart: ChartNoAxesCombined, radio: Radio, shield: ShieldCheck, wrench: Wrench, "clipboard-check": ClipboardCheck };
const ICON_LABELS = { camera: "Camera", "map-pin": "Định vị", "file-spreadsheet": "Bảng tính", gauge: "Tốc độ", headphones: "Hỗ trợ", video: "Video", bus: "Xe buýt", chart: "Biểu đồ", radio: "Giám sát", shield: "Kiểm tra", wrench: "Sửa chữa", "clipboard-check": "Hậu kiểm" };
...
function BoxEditor({ box, boxes, onClose, onSave }) {
  ...
  const core = isCoreReportBox(form);
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/60 backdrop-blur-sm sm:items-center sm:p-5"><form onSubmit={submit} className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-t-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 sm:rounded-3xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/95 px-6 py-5 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95"><div><h2 className="text-lg font-bold text-ink dark:text-white">{box ? "Chỉnh sửa Box báo cáo" : "Thêm Box báo cáo"}</h2><p className="mt-1 text-xs text-slate-500">Tên, mô tả, video và trạng thái sẽ cập nhật trên trang chính.</p></div><button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-xl text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"><X size={20}/></button></div>
    <div className="grid gap-5 p-6 sm:grid-cols-2">
      ...
      <label><span className="field-label">Slug</span><input className="field-input disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 dark:disabled:bg-slate-800" value={form.slug} disabled={core} onChange={event => set("slug", event.target.value)} placeholder="bao-cao-ten-moi"/><small className="mt-1.5 block text-xs text-slate-400">{core ? "BOX hệ thống: slug và đường dẫn chức năng được khóa để tránh mất chức năng khi chỉnh màu/video." : `/bao-cao/${slugifyBlogTitle(form.slug || form.title) || "slug"}`}</small></label>
...
export default function ReportBoxAdminPage() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.17em] text-violet-600 dark:text-violet-300"><Clapperboard size={16}/>Nội dung trang chính</span><h1 className="mt-3 text-3xl font-bold tracking-tight text-ink dark:text-white sm:text-4xl">Quản lý Box báo cáo tuần</h1><p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">Thêm, sửa, xóa, sắp xếp, ẩn Box và cập nhật video hướng dẫn Google Drive. {CORE_REPORT_BOX_COUNT} Box chính được khóa xóa và giữ nguyên đường dẫn chức năng.</p></div><button className="primary-button" onClick={() => { setEditor(null); setEditorOpen(true); }}><Plus size={18}/>Thêm Box</button></div>
    ...
    {deleteTarget && <ConfirmDialog danger title="Xóa Box báo cáo?" description={`“${deleteTarget.title}” sẽ bị xóa vĩnh viễn. Thao tác này không ảnh hưởng các Box báo cáo chính.`} confirmLabel="Xóa Box" busy={deleting} onClose={() => setDeleteTarget(null)} onConfirm={remove}/>}
  </section>;
}
