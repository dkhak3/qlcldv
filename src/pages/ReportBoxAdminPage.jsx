...
export default function ReportBoxAdminPage() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.17em] text-violet-600 dark:text-violet-300"><Clapperboard size={16}/>Nội dung trang chính</span><h1 className="mt-3 text-3xl font-bold tracking-tight text-ink dark:text-white sm:text-4xl">Quản lý Box báo cáo tuần</h1><p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">Thêm, sửa, xóa, sắp xếp, ẩn Box và cập nhật video hướng dẫn Google Drive. Bảy Box chính được khóa xóa.</p></div><button className="primary-button" onClick={() => { setEditor(null); setEditorOpen(true); }}><Plus size={18}/>Thêm Box</button></div>
    ...
    {deleteTarget && <ConfirmDialog danger title="Xóa Box báo cáo?" description={`“${deleteTarget.title}” sẽ bị xóa vĩnh viễn. Thao tác này không ảnh hưởng 7 Box báo cáo chính.`} confirmLabel="Xóa Box" busy={deleting} onClose={() => setDeleteTarget(null)} onConfirm={remove}/>}
  </section>;
}
