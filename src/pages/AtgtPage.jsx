...
function SummaryTable({ rows }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
    <table className="w-full min-w-[780px] border-collapse text-sm">
      ...
      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">{rows.map(row => <tr key={`${row.branch}-${row.route}`} className="hover:bg-indigo-50/40 dark:hover:bg-indigo-950/20"><td className="px-3 py-3 text-center text-slate-500">{row.stt}</td><td className="px-3 py-3 font-semibold text-slate-700 dark:text-slate-200">{row.branch}</td><td className="px-3 py-3 text-slate-600 dark:text-slate-300">{row.route}</td><td className="px-3 py-3 text-center font-bold text-indigo-700 dark:text-indigo-300">{row.total}</td><td className="px-3 py-3 text-center">{row.finance}</td><td className="px-3 py-3 text-center">{row.atgt}</td><td className="px-3 py-3 text-center">{row.cldv}</td></tr>)}</tbody>
      <tfoot><tr className="border-t border-indigo-100 bg-indigo-50 font-bold dark:border-indigo-900 dark:bg-indigo-950/30"><td className="px-3 py-3 text-center text-indigo-700 dark:text-indigo-300" colSpan="3">Tổng</td><td className="px-3 py-3 text-center">{totals.total}</td><td className="px-3 py-3 text-center">{totals.finance}</td><td className="px-3 py-3 text-center">{totals.atgt}</td><td className="px-3 py-3 text-center">{totals.cldv}</td></tr></tfoot>
    </table>
  </div>;
}
...
function ViolationTable({ rows, emptyMessage = "Không có vi phạm trong các file đã chọn." }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700"><table className="w-full min-w-[1540px] border-collapse text-sm"><thead className="bg-slate-50 text-slate-600 dark:bg-slate-950/70 dark:text-slate-300"><tr><th className="px-3 py-3 text-center">STT</th><th className="px-3 py-3 text-left">Nhóm</th><th className="px-3 py-3 text-left">Chi nhánh</th><th className="px-3 py-3 text-left">Tuyến</th><th className="px-3 py-3 text-left">Số xe</th><th className="px-3 py-3 text-left">Họ và tên</th><th className="px-3 py-3 text-left">Nội dung vi phạm M03</th><th className="px-3 py-3 text-center">Kết quả</th><th className="px-3 py-3 text-left">Dữ liệu M02 đối chiếu</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{rows.map(row => {
    ...
    return <tr key={`${row.sourceFile}-${row.sourceRow}-${row.stt}`} className="align-top hover:bg-indigo-50/30 dark:hover:bg-indigo-950/20"><td className="px-3 py-3 text-center text-slate-500">{row.stt}</td><td className="px-3 py-3 font-semibold text-indigo-700 dark:text-indigo-300">{row.categoryLabel}</td><td className="px-3 py-3 text-slate-600 dark:text-slate-300">{row.branch}</td><td className="max-w-xs px-3 py-3 text-slate-600 dark:text-slate-300">{row.route}</td><td className="px-3 py-3 font-semibold text-slate-700 dark:text-slate-200">{row.vehicle}</td><td className="px-3 py-3 text-slate-600 dark:text-slate-300">{row.employeeName || "—"}</td><td className="max-w-md whitespace-pre-wrap px-3 py-3 text-slate-600 dark:text-slate-300">{row.violation}</td><td className="px-3 py-3 text-center">{row.matchedM02 ? <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 text-xs font-bold text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300"><CheckCircle2 size={13}/>Trùng khớp</span> : <span className="inline-flex rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-700 dark:bg-amber-950/60 dark:text-amber-300">Chưa thấy trong M02</span>}</td><td className="w-[390px] px-3 py-3">{matched ? <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3 text-xs leading-5 text-slate-600 dark:border-indigo-900/60 dark:bg-indigo-950/25 dark:text-slate-300"><div className="flex flex-wrap gap-x-4 gap-y-1"><b className="text-indigo-700 dark:text-indigo-300">Ngày {formatDate(matched.date)}</b><span>Dòng M02: {matched.sourceRow}</span></div><p className="mt-1"><b>Lái xe:</b> {matched.driver || "—"} · <b>Tiếp viên:</b> {matched.assistant || "—"}</p><p className="mt-2 whitespace-pre-wrap break-words border-t border-indigo-100 pt-2 dark:border-indigo-900/60"><b>Ghi chú M02:</b> {matched.note || "—"}</p><p className="mt-2 truncate text-[10px] text-slate-400" title={matched.sourceFile}>{matched.sourceFile}</p></div> : <div className="rounded-xl border border-amber-100 bg-amber-50/60 p-3 text-xs leading-5 text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/25 dark:text-amber-300"><p>Không tìm thấy dòng M02 có cùng chi nhánh, tuyến, số xe, nhân viên và nội dung ghi chú. Dòng M03 này vẫn được ghi nhận trong báo cáo.</p><p className="mt-2 break-words border-t border-amber-200 pt-2 text-[11px] text-amber-800 dark:border-amber-900/70 dark:text-amber-200"><b>File ATGT:</b> {row.sourceFile || "—"}</p></div>}</td></tr>;
  })}</tbody></table></div>;
}

export default function AtgtPage() {
  ...
  const chooseFiles = async event => {
    ...
    try {
      ...
      for (const file of candidates) {
        const fingerprint = await fingerprintAtgtFile(file).catch(() => `${file.name.toLowerCase()}-${file.size}-${file.lastModified}`);
      ...
      if (accepted.length) {
        ...
        resetResults();
        toast.success(`Đã thêm ${accepted.length} file ATGT`);
      }
  ...
  const search = async () => {
    ...
    resetResults();
    try {
      const data = await processAtgtFiles(files.map(item => item.file));
      setResults(data);
      setProcessed(true);
      if (data.detailRows.length || data.violations.length) {
        toast.success(`Hoàn tất! ${data.detailRows.length} lượt giám sát ATGT, ${data.violations.length} ghi nhận vi phạm`);
      ...
      } else toast.info("Các file hợp lệ nhưng không có dữ liệu giám sát ATGT");
    } catch (error) {
      setResults(EMPTY_RESULTS);
      setProcessed(false);
      toast.error(error.message || "Không thể đọc file ATGT");
  ...
  const download = async () => {
    setLoading(true);
    try {
      await exportAtgtReport({ results, startDate, endDate, employees });
      toast.success("Báo cáo ATGT đã được tạo theo đúng biểu mẫu");
    } catch (error) { toast.error(error.message || "Không thể xuất báo cáo ATGT"); }
    finally { setLoading(false); }
  };
  ...
  return <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
    <div className="mx-auto mb-10 max-w-3xl text-center"><span className="inline-flex rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-bold tracking-[.16em] text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">BÁO CÁO TUẦN</span><h1 className="mt-4 text-3xl font-bold tracking-tight text-ink dark:text-white sm:text-4xl">Tổng hợp báo cáo ATGT</h1><p className="mt-3 text-sm leading-6 text-slate-500 dark:text-slate-400 sm:text-base">Gộp từ 1 đến 7 file giám sát ATGT hằng ngày, đối chiếu M02/M03 và xuất báo cáo tuần theo mẫu CITYBUS.</p></div>
    <div className="grid items-start gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <ManagedGuideVideo reportKey="atgt" description="Cách chuẩn bị file và xuất báo cáo ATGT"/>
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        <div className="flex items-start gap-3 border-b border-slate-100 pb-5 dark:border-slate-800"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300"><FileSpreadsheet size={20}/></span><div><h2 className="font-bold text-ink dark:text-white">Dữ liệu báo cáo ATGT</h2><p className="mt-1 text-xs text-slate-500">Chọn từ 1 đến 7 file hằng ngày; mỗi file phải có sheet M02 và M03</p></div></div>
    ...
    <div className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900 sm:p-6"><div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300"><ShieldCheck size={20}/></span><div><h2 className="text-lg font-bold text-ink dark:text-white">Kết quả tổng hợp ATGT</h2><p className="mt-1 text-xs leading-5 text-slate-500">M03 có dữ liệu nhưng không trùng ghi chú M02 vẫn được ghi nhận. Mục IV được hiển thị riêng trên web để kiểm tra và không được đưa vào file Excel báo cáo.</p></div></div>{hasData && <button className="secondary-button !h-10 shrink-0" onClick={download} disabled={loading}><Download size={17}/>Tải Excel</button>}</div>
      {loading && !processed ? <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl bg-slate-50 text-slate-500 dark:bg-slate-950/50"><LoaderCircle className="animate-spin text-indigo-700" size={34}/><span className="mt-4 text-sm font-semibold">Đang đọc và đối chiếu M02/M03...</span></div> : !hasData ? <NoData searched={processed}/> : <div className="space-y-8"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-950/50"><span className="text-xs font-semibold text-slate-400">FILE ĐÃ NHẬP</span><b className="mt-2 block text-2xl text-slate-800 dark:text-white">{results.fileSummaries.length}</b></div><div className="rounded-xl bg-indigo-50 p-4 dark:bg-indigo-950/30"><span className="text-xs font-semibold text-indigo-600">LƯỢT GIÁM SÁT</span><b className="mt-2 block text-2xl text-indigo-800 dark:text-indigo-300">{results.detailRows.length}</b></div><div className="rounded-xl bg-rose-50 p-4 dark:bg-rose-950/30"><span className="text-xs font-semibold text-rose-600">GHI NHẬN VI PHẠM</span><b className="mt-2 block text-2xl text-rose-700 dark:text-rose-300">{results.violations.length}</b></div><div className="rounded-xl bg-amber-50 p-4 dark:bg-amber-950/30"><span className="text-xs font-semibold text-amber-600">M03 CHƯA KHỚP M02</span><b className="mt-2 block text-2xl text-amber-700 dark:text-amber-300">{results.unmatchedCount}</b></div></div>
        <section><h3 className="mb-3 text-sm font-bold text-indigo-700 dark:text-indigo-300">1. Các file đã xử lý</h3><div className="grid gap-3 sm:grid-cols-2">{results.fileSummaries.map(item => <div key={item.fileName} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex items-start gap-3"><FileCheck2 className="mt-0.5 shrink-0 text-indigo-600" size={19}/><div className="min-w-0"><b className="block truncate text-sm text-slate-700 dark:text-slate-200">{item.fileName}</b><span className="mt-1 block text-xs text-slate-400">{item.detailCount} lượt · {item.violationCount} vi phạm · {item.dates.length ? item.dates.map(formatDate).join(", ") : "không tìm thấy ngày trong file"}</span></div></div></div>)}</div></section>
        <section><h3 className="mb-3 text-sm font-bold text-indigo-700 dark:text-indigo-300">2. BCTH.HKVP — Tổng hợp theo Chi nhánh/Tuyến</h3>{results.summary.length ? <SummaryTable rows={results.summary}/> : <NoData searched/>}</section>
        <section><h3 className="mb-3 text-sm font-bold text-indigo-700 dark:text-indigo-300">3. Đối chiếu vi phạm M03 với ghi chú M02</h3><ViolationTable rows={reportViolations} emptyMessage="Không có vi phạm thuộc nhóm I, II hoặc III trong các file đã chọn."/></section>
