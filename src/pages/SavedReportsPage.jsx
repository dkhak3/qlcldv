...
function HauKiemDetail({ report }) {
  ...
  const label = isAtgt ? "ATGT" : "Hậu kiểm";
  const accent = isAtgt ? "text-indigo-700 dark:text-indigo-300" : "text-emerald-700 dark:text-emerald-300";
  return <section><h3 className={`mb-3 text-sm font-bold ${accent}`}>Tổng hợp {label} theo Chi nhánh/Tuyến</h3><div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700"><table className="w-full min-w-[780px] border-collapse text-sm"><thead className="bg-slate-50 text-slate-600 dark:bg-slate-950/70 dark:text-slate-300"><tr><th className="px-3 py-3 text-center">STT</th><th className={headClass}>Chi nhánh</th><th className={headClass}>Tuyến</th><th className="px-3 py-3 text-center">Số lượng vi phạm</th><th className="px-3 py-3 text-center">Tài chính</th><th className="px-3 py-3 text-center">ATGT</th><th className="px-3 py-3 text-center">CLDV</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{rows.map((row, index) => <tr key={`${row.branch}-${row.route}-${index}`}><td className="px-3 py-3 text-center text-slate-500">{row.stt || index + 1}</td><td className={cellClass}>{row.branch || "—"}</td><td className={cellClass}>{row.route || "—"}</td><td className={`px-3 py-3 text-center font-bold ${accent}`}>{Number(row.total) || 0}</td><td className="px-3 py-3 text-center">{Number(row.finance) || 0}</td><td className="px-3 py-3 text-center">{Number(row.atgt) || 0}</td><td className="px-3 py-3 text-center">{Number(row.cldv) || 0}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs text-slate-400">{report.data?.detailRows?.length || 0} lượt {isAtgt ? "giám sát" : "hậu kiểm"} · {report.data?.violations?.length || 0} ghi nhận vi phạm · {report.data?.unmatchedCount || 0} ghi nhận M03 chưa khớp ghi chú M02</p></section>;
}
