...
export default function HomePage() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      ...
      <div className="relative grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.2fr_.8fr] lg:gap-7 lg:p-8 xl:p-10">
        <div className="flex min-h-[350px] flex-col justify-between rounded-[26px] border border-white/80 bg-white/72 p-6 shadow-sm backdrop-blur-xl dark:border-white/5 dark:bg-slate-950/45 sm:p-8">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-3xl font-bold leading-[1.28] tracking-[-0.025em] text-ink dark:text-white sm:text-4xl sm:leading-[1.24] xl:text-[42px] xl:leading-[1.22]">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          <button onClick={copyWeek} className="group relative overflow-hidden rounded-[24px] border border-orange-100 bg-white/90 p-5 text-left shadow-card backdrop-blur transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-xl dark:border-slate-700 dark:!bg-slate-900 dark:hover:border-orange-500/50 dark:hover:!bg-slate-800">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white/90 p-5 shadow-card backdrop-blur dark:border-slate-700 dark:!bg-slate-900">
    ...
    {loadingCards ? <div className="flex min-h-56 items-center justify-center"><LoaderCircle className="animate-spin text-brand-500" size={32}/></div> : cards.length ? <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{cards.map(card => {
      const Icon = ICONS[card.icon] || FileSpreadsheet;
      const body = <><span className={`grid h-12 w-12 place-items-center rounded-2xl ${REPORT_APPEARANCES[card.appearance] || REPORT_APPEARANCES.orange}`}><Icon size={25}/></span><h3 className="mt-6 text-lg font-bold text-ink transition-colors dark:text-white dark:group-hover:text-orange-100">{card.title}</h3><p className="mt-2 min-h-11 text-sm leading-6 text-slate-500 transition-colors dark:text-slate-400 dark:group-hover:text-slate-200">{card.description}</p><span className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-brand-600 dark:text-orange-300 dark:group-hover:text-orange-200">Mở báo cáo <ArrowRight size={17} className="transition group-hover:translate-x-1"/></span></>;
      const className = "group rounded-2xl border border-slate-200 bg-white p-6 shadow-card transition duration-200 hover:-translate-y-1 hover:border-orange-200 hover:bg-orange-50/40 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900 dark:hover:-translate-y-1 dark:hover:border-orange-400/60 dark:hover:bg-slate-800 dark:hover:shadow-[0_18px_44px_-18px_rgba(249,115,22,.35)]";
