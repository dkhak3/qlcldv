...
export default function BlogDetailPage() {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8">
    ...
    <article className="mx-auto mt-7 max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
      <BlogCover post={post} className="aspect-[16/9] max-h-[560px] w-full" />
      <div className="px-6 py-8 sm:px-12 sm:py-12 lg:px-16">
        ...
        <h1 className="mt-5 break-words text-3xl font-bold leading-[1.3] tracking-tight text-ink dark:text-white sm:text-4xl sm:leading-[1.28]">{post.title}</h1>
        <p className="mt-4 text-base leading-7 text-slate-500 dark:text-slate-400 sm:text-lg">{post.excerpt}</p>
        <div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 border-y border-slate-100 py-5 text-xs font-medium text-slate-500 dark:border-slate-800 dark:text-slate-400"><BlogAuthorLink post={post} className="flex items-center gap-1.5"><UserRound size={15}/>{post.author}</BlogAuthorLink><span className="flex items-center gap-1.5"><CalendarDays size={15}/>{formatBlogDate(post.publishedAt || post.createdAt)}</span><span className="flex items-center gap-1.5"><Clock3 size={15}/>{post.readTime} phút đọc</span><span className="flex items-center gap-1.5"><Eye size={15}/>{post.views} lượt xem</span></div>
        ...
        <div className="mt-9"><BlogArticleContent content={post.content}/></div>
        <BlogEngagementBar post={post}/>
