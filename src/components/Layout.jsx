...
import { Activity, BookMarked, BookOpenText, ChevronDown, ClipboardCheck, Clapperboard, FileClock, FileSpreadsheet, Gauge, HandCoins, Home, Link2, LogIn, LogOut, Moon, PartyPopper, Settings2, Sun, Tags, Trophy, UsersRound } from "lucide-react";
...
import TetThemeDecor, { TetThemeBanner } from "./TetThemeDecor";
...
export default function Layout({ children }) {
  ...
  return <div className={`flex min-h-screen flex-col bg-slate-50 transition-colors duration-300 dark:bg-slate-950 ${tet.enabled ? "tet-app-shell" : ""}`}>
    <TetThemeDecor />
    <header className="tet-site-header sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl transition-colors dark:border-slate-800 dark:bg-slate-950/90">
      <TetThemeBanner />
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to="/" className="group flex shrink-0 items-center gap-3 text-ink no-underline dark:text-white"><span className="relative grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-brand-500 to-amber-400 text-white shadow-lg shadow-orange-200 transition group-hover:scale-105 dark:shadow-orange-950/40"><ClipboardCheck size={24}/>{tet.enabled && <span className="absolute -right-1.5 -top-1.5 grid h-5 w-5 place-items-center rounded-full bg-red-600 text-[11px] shadow ring-2 ring-white dark:ring-slate-950">🌸</span>}</span><span className="hidden min-[420px]:block"><b className="block text-lg font-bold tracking-tight">QLCL-DV</b><small className="hidden text-[11px] font-medium text-slate-500 dark:text-slate-400 sm:block">Hệ thống báo cáo tuần</small></span></Link>
