...
export default function HauKiemPage() {
  ...
  const valid = files.length > 0 && startDate && endDate && employees.trim() && startDate <= endDate;
  ...
  const search = async () => {
    ...
    resetResults();
    try {
      const data = await processHauKiemFiles(files.map(item => item.file), startDate, endDate);
      setResults(data);
      setProcessed(true);
  ...
  return <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
    ...
    <div className="grid items-start gap-6 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><label><span className="field-label"><CalendarDays size={17}/>Từ ngày</span><input className="field-input report-input report-input-emerald" type="date" value={startDate} onChange={event => { setStartDate(event.target.value); resetResults(); }}/></label><label><span className="field-label"><CalendarDays size={17}/>Đến ngày</span><input className="field-input report-input report-input-emerald" type="date" min={startDate} value={endDate} onChange={event => { setEndDate(event.target.value); resetResults(); }}/></label></div>
