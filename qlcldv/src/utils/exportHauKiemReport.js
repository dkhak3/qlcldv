...
function patchSummarySheet(sheetXml, summary, startDate, endDate) {
  ...
  const templateDataRow = getRow(rows, 7);
  ...
  const totalRowNumber = 13 + delta;
  const totalRow = rewriteRow(getRow(rows, 13), totalRowNumber, {
    A: "Tổng",
    D: { kind: "formula", formula: `SUM(D7:D${totalRowNumber - 1})`, result: summary.reduce((sum, item) => sum + (Number(item.total) || 0), 0) },
    E: { kind: "formula", formula: `SUM(E7:E${totalRowNumber - 1})`, result: summary.reduce((sum, item) => sum + (Number(item.finance) || 0), 0) },
    F: { kind: "formula", formula: `SUM(F7:F${totalRowNumber - 1})`, result: summary.reduce((sum, item) => sum + (Number(item.atgt) || 0), 0) },
    G: { kind: "formula", formula: `SUM(G7:G${totalRowNumber - 1})`, result: summary.reduce((sum, item) => sum + (Number(item.cldv) || 0), 0) },
  });
  const footerRows = rows
    .filter(row => rowNumberOf(row) >= 14)
  ...
  output = replaceDimension(output, `A1:G${22 + delta}`);
  return shiftMerges(output, 13, delta);
}
...
function patchDetailSheet(sheetXml, detailRows, employees, startDate, endDate) {
  ...
  headerRows[2] = rewriteRow(headerRows[2], 3, {
